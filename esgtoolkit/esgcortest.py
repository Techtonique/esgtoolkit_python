...
from .config import R_IS_INSTALLED, RPY2_IS_INSTALLED, RPY2_ERROR_MESSAGE, \
    USAGE_MESSAGE, ESGTOOLKIT_PACKAGE, FLOATMATRIX, FLOATVECTOR, STRVECTOR, R_NULL
...
def esgcortest(
  ...
  return_R_obj = False
    ...
    if return_R_obj:
        return ESGTOOLKIT_PACKAGE.esgcortest(x, 
                                         alternative = alternative,
                                         method = method,
                                         conf_level = conf_level)

    # return a dictionary of dataframes
    res  = ESGTOOLKIT_PACKAGE.esgcortest(x, 
                                         alternative = alternative,
                                         method = method,
                                         conf_level = conf_level)    
    res_dict = {}
    res_dict["cor.estimate"] = pd.DataFrame(np.array(res.rx2("cor.estimate")), columns=["cor.estimate"], 
                                            index=r.time(res.rx2("cor.estimate")))
    res_dict["conf.int"] = pd.DataFrame(np.array(res.rx2("conf.int")), columns=["lower", "upper"], 
                                        index=r.time(res.rx2("cor.estimate"))) 
